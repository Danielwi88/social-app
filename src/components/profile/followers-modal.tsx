...
export function FollowersModal({ username, isOpen, onClose, isMe = false }: FollowersModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
      <div className="w-full max-w-md rounded-3xl border border-white/10 bg-[#12121d]/95 p-6">
        ...
        <div className="max-h-96 overflow-y-auto">
          {isLoading ? (
            <p className="text-white/70 text-center py-8">Loading followers...</p>
          ) : isError ? (
            <p className="text-rose-400 text-center py-8">Failed to load followers</p>
              ...
              {followers.map((follower: PublicUser) => (
                <div key={follower.id} className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    ...
                    />
                    <div>
                      <p className="text-sm font-medium text-white">{follower.displayName}</p>
                  ...
                  <FollowButton
                    ...
                    followersCount={0}
                    disabled={follower.isMe || follower.username === me?.username}
