...
export function FollowingModal({ username, isOpen, onClose, isMe = false }: FollowingModalProps) {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
      <div className="w-full max-w-md rounded-3xl border border-white/10 bg-[#12121d]/95 p-6">
        ...
        <div className="max-h-96 overflow-y-auto">
          {isLoading ? (
            <p className="text-white/70 text-center py-8">Loading following...</p>
          ) : isError ? (
            <p className="text-rose-400 text-center py-8">Failed to load following</p>
          ) : !following || following.length === 0 ? (
            <p className="text-white/60 text-center py-8">Not following anyone yet</p>
          ) : (
