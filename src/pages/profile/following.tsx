...
export default function PublicFollowing() {
  ...
  if (profile.isLoading || q.isLoading) return <p className="text-white/70">Loading…</p>;
  if (profile.isError || !profile.data) return <p className="text-rose-400">User not found.</p>;
  if (q.isError) return <p className="text-rose-400">Failed to load following.</p>;
      ...
      <header className="flex items-center justify-between">
        <div>
          ...
          <div className="text-white/70">
            for <Link to={`/profile/${u.username}`} className="underline">@{u.username}</Link>
          </div>
        </div>
      </header>
